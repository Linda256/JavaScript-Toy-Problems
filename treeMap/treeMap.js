...
Tree.prototype.map = function (func) {
  // create a new tree newTree using the value from  calling function func   
 var newTree = new Tree(func(this.value));
// if currNode has children, loop through each child
 if (this.children){
    for (var i=0 ;  i<this.children.length ; i++){
    // recursively call map for each child
...
Tree.prototype.mapInPlace = function (func) {
  // call this.map(func) to get newTree with updated value.
  var newTree = new Tree(func(this.value));
  var newVal = func(this.value);
  this.value = newVal;
 if (this.children){
    for (var i=0 ;  i<this.children.length ; i++){
       newTree.children.push(this.children[i].mapInPlace(func));
    }
  }
  return this;
